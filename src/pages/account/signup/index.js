...
export default function Signup({ history }) {
  ...
  const { signUp, kakaoSignUp, isAuthenticated } =
  ...
  const onSubmit = (data) =>
    isEmailValid && isPasswordValid && isReEnterPasswordValid && signUp(data);
  const validateEmail = (emailEntered) => {
    const emailRegex =
      /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i;
    setValue("email", emailEntered);
    emailEntered.match(emailRegex)
  ...
  const validatePassword = (passwordEntered) => {
    ...
    passwordEntered.match(passwordRegex)
      ? setIsPasswordValid(true)
      : setIsPasswordValid(false);
  };

  const validateReEnterPassword = (passwordReEntered) => {
    passwordReEntered === passwordEntered
  ...
  const formCheck = () => {
    ...
    !isPasswordValid &&
      message.warning("비밀번호를 형식에 맞게 입력해주세요 🥺");
